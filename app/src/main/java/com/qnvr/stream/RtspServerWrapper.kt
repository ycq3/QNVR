...
class RtspServerWrapper(
    ...
    private val enableNetworkOptimization: Boolean = true
) {
    ...
    private fun enterLowPowerMode() {
        ...
        camera.setFps(5)
        fps = 5
        val lowBitrate = (originalBitrate * 0.3).toInt().coerceAtLeast(10000)
        bitrate = lowBitrate
        if (::encoder.isInitialized) {
            try {
                encoder.setBitrateManually(lowBitrate)
            } catch (e: Exception) {
                android.util.Log.w("RtspServerWrapper", "Failed to reduce encoder bitrate in low power mode", e)
            }
        }
    }
    ...
    private fun exitLowPowerMode() {
        ...
        lowPowerMode = false
        camera.setFps(originalFps)
        fps = originalFps
        bitrate = originalBitrate
        if (::encoder.isInitialized) {
            try {
                encoder.setBitrateManually(originalBitrate)
            } catch (e: Exception) {
                android.util.Log.w("RtspServerWrapper", "Failed to restore encoder bitrate", e)
            }
        }
    }
        ...
        while (true) {
            ...
            when (method) {
                ...
                "SETUP" -> {
                    ...
                    try {
                        networkOptimizer?.optimizeSocketBuffer(socket)
                    } catch (e: Exception) {
                        android.util.Log.w("RtspServerWrapper", "Failed to optimize socket buffer", e)
                    }
                }
                "PLAY" -> {
    ...
    private fun streamLoop(out: OutputStream, channel: Int, queue: java.util.concurrent.LinkedBlockingQueue<com.qnvr.stream.VideoEncoder.EncodedFrame>) {
        ...
        while (true) {
            try {
                ...
                frameCount++
                if (frameCount % 60 == 0 || frame.keyframe) {
                    android.util.Log.d("RtspServerWrapper", "Streamed $frameCount frames, key=${frame.keyframe}, size=${frame.data.size}, nals=${nals.size}")
                }
                for (nal in nals) {
                    if (nal.isNotEmpty()) {
